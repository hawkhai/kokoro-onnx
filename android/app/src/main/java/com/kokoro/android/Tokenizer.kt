 ...
 *
 * Converts phonemes to token IDs using the vocabulary mapping.
 */
class Tokenizer(private val vocab: Map<String, Int> = DEFAULT_VOCAB) {

    companion object {
        /**
         * Default vocabulary loaded from config.json.
         * Maps phoneme characters to integer token IDs.
         */
        val DEFAULT_VOCAB: Map<String, Int> = mapOf(
            ...
            "χ" to 142, "ʎ" to 143, "ʒ" to 147, "ʔ" to 148, "ˈ" to 156,
            "ˌ" to 157, "ː" to 158, "ʰ" to 162, "ʲ" to 164, "↓" to 169,
            "→" to 171, "↗" to 172, "↘" to 173, "ᵻ" to 177
        )
    }
    ...
    /**
     * Convert phonemes string to list of token IDs.
     * Characters not in vocabulary are silently skipped.
     */
    fun tokenize(phonemes: String): List<Int> {
        if (phonemes.length > KokoroConfig.MAX_PHONEME_LENGTH) {
            throw IllegalArgumentException(
                "Text is too long, must be less than ${KokoroConfig.MAX_PHONEME_LENGTH} phonemes"
            )
        }
        return phonemes.mapNotNull { vocab[it.toString()] }
    }
}
